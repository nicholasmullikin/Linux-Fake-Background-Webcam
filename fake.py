...
class FakeCam:
    def __init__(self, args) -> None:
        ...
        self.classifier = ImageSegmenter.create_from_options(
        ...
        self.old_mask = None
        self.real_cam = RealCam(
            self.webcam_path, self.width, self.height, self.fps, self.codec
        )
        ...
        self.fake_cam = pyfakewebcam.FakeWebcam(
            self.v4l2loopback_path, self.width, self.height
        )
    ...
    def compose_frame(self, frame):
        ...
        if self.cmap_bg:
            cv2.applyColorMap(
                background_frame, cmap(self.cmap_bg), dst=background_frame
            )
